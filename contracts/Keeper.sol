...
contract BattleRoyaleArena is VRFConsumerBase {
     ...
     */
    function getCurrentBalance() external view onlySupport returns (uint256) {
        uint256 balance = address(this).balance;
        return balance;
    }
     ...
     */
    function getCurrentLinkBalance()
        external
        view
        onlySupport
        returns (uint256)
    ...
    function addToBattleQueue(address payable _nftAddress)
        external
        payable
        onlySupport
        returns (bool)
    ...
    function isContractInQueue(address payable _contract)
        external
        view
        returns (bool)
    {
    ...
    function removeFromQueue(address payable nftAddress)
        external
        payable
        onlySupport
    ...
    function setWalletAddress(address payable _wallet)
        external
        payable
        onlyOwner
    {
     ...
     */
    function addressToBytes(address payable a)
        internal
        pure
    ...
    {
        for (uint256 i = 0; i < battleQueue.size(); i++) {
    ...
    function executeBattle(address payable _nftAddress)
        internal
        returns (bytes32)
    {
